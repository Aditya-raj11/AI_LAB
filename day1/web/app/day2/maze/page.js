...
function* searchTrace(maze, start, goal, algorithm) {
  const key = (r,c) => `${r},${c}`;
  const isGBFS = algorithm === "GBFS";
  const useQueue = algorithm === "BFS";

  const manhattan = (p, q) => Math.abs(p[0] - q[0]) + Math.abs(p[1] - q[1]);
  ...
  while (frontier.length) {
    ...
    if (isGBFS) {
      ...
      for (let i = 0; i < frontier.length; i++) {
        const h = manhattan(frontier[i][0], goal);
        if (h < minH) { minH = h; minIdx = i; }
      }
      [node, path] = frontier.splice(minIdx, 1)[0];
    } else {
      [node, path] = useQueue ? frontier.shift() : frontier.pop();
    }
    const nk = key(...node);
    visited.add(nk);
    step++;
...
export default function MazePage() {
  ...
  const [algorithm, setAlgorithm] = useState("GBFS");
